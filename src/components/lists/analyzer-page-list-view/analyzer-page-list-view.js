...
const AnalyzerPageListView = (props) => {
    ...
    const { anal } = props
    const releases = anal ? anal.singles ? {singles: anal.singles, albums: anal.albums} : null : null
    const accounts = anal ? anal.artist.links.length > 0 ? anal.artist.links : null : null
    const videos = anal ? anal.videos.count ? anal.videos : null : null
    const genres = anal ? anal.genres : null
    const top = anal ? anal.top : null
    const charts = anal ? anal.charts.all_time ? anal.charts : null : null
    const chartsTop = anal ? anal.charts.all_time ? {days: anal.charts.top_by_days, positions: anal.charts.top_by_positions} : null : null
    const chartsStats = anal ? anal.charts.days_medians ? {days: anal.charts.days_medians, positions: anal.charts.positions_medians} : null : null
